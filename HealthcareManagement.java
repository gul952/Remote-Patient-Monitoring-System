...
// -------------------------------
// Original Healthcare Management System Code
// -------------------------------
...
public class HealthcareManagement {
    ...
    public static void main(String[] args) {
        ...
        while (mainLoop) {
            ...
            switch (choice) {
                case 1:
                    // Patient workflow: the patient enters their ID and stays in the patient menu until they choose to go back.
                    ...
                    while (patientMenu) {
                        ...
                        switch (pChoice) {
                            case 1:
                                // Patient enters vitals. We read each vital sign from the console.
                                try {
                                ...
                                break;
                            case 2:
                                // Patient can schedule or view appointments.
                                currentPatient.manageAppointments(appointmentManager, scanner);
                                break;
                            case 3:
                                // Display feedback and prescription details.
                                currentPatient.viewFeedbackAndPrescription();
                                break;
                            case 4:
                                // Show the patient's medical history.
                            ...
                            case 6:
                                ...
                                break;
                            case 7:
                                // Exit the patient menu and return to the main menu.
                                patientMenu = false;
                                break;
                            default:
                                System.out.println("Invalid choice.");
                ...
                case 2:
                    // Doctor workflow: the doctor enters their ID and stays in their menu.
                    ...
                    while (doctorMenu) {
                        ...
                        switch (dChoice) {
                            case 1:
                                // Show patient IDs for all approved appointments.
                                currentDoctor.viewPatientIDs();
                                break;
                            case 2:
                                // Allow the doctor to manage a specific patient's data.
                ...
                case 3:
                    // Administrator workflow: We now create an Administrator that extends User.
                    // The admin does not use an ID, so we pass an empty string.
                    ...
                    while (adminMenu) {
                        ...
                        switch (aChoice) {
                            case 1:
                                // Manage doctors by performing actions like add, remove, or update.
                            ...
                            case 3:
                                ...
                                break;
                            case 4:
                                // Display all registered Patient and Doctor IDs.
                                admin.viewAllIDs(patientDb, doctorDb);
                                break;
                            case 5:
                                // Exit administrator menu.
                                adminMenu = false;
                                break;
                            default:
                                System.out.println("Invalid choice.");
                ...
                case 4:
                    // Extra Features Menu: Integrates the new modules (EmergencyAlert, PanicButton, Chat & Video, Reminders)
    ...
    private static void runExtraFeatures(Scanner scanner) {
        // Create instances of new modules
        ...
        while (extraLoop) {
            ...
            switch (extraChoice) {
                case 1:
                    // EmergencyAlert: get vitals input from the user and check for alerts.
                    System.out.print("Enter heart rate: ");
                    int hr = Integer.parseInt(scanner.nextLine());
                ...
                case 3:
                    ...
                    // Use the overloaded method to send message from sender to receiver.
                    chatClient.sendMessage(sender, receiver, message);
                    // Display chat history.
                    chatServer.showMessages();
                    break;
                case 4:
                    // VideoCall: simulate starting a video call.
                    System.out.print("Enter patient name: ");
...
// -------------------------------
// 6. New Modules and Classes to be Integrated
// -------------------------------
...
// sendNotification() just prints the message to simulate delivery.
...
class NotificationService {
    ...
    public void sendEmailAlert(String recipient, String message) {
        ...
        // Store the notification in the recipient's inbox/alerts.
        HealthcareManagement.addNotification(recipient, "Email Alert: " + message);
    }

    public void sendSMSAlert(String recipient, String message) {
        smsNotifier.sendNotification(recipient, message);
        // Store the notification in the recipient's inbox/alerts.
        HealthcareManagement.addNotification(recipient, "SMS Alert: " + message);
    }
}
...
class EmergencyAlert {
    ...
    // Checks given vitals and triggers alert if thresholds are exceeded.
...
// Simulates message passing (console-based) using an ArrayList to store messages.
class ChatServer {
...
class ChatClient {
    ...
    public void sendMessage(String sender, String receiver, String message) {
        ...
        server.receiveMessage(fullMessage);
        // Store the chat message for both sender and receiver.
        HealthcareManagement.addChatMessage(sender, fullMessage);
        HealthcareManagement.addChatMessage(receiver, fullMessage);
    }
...
class ReminderService {
    ...
    public void sendAppointmentReminder(String recipient, String time) {
        ...
        smsNotifier.sendNotification(recipient, message);
        // Store the reminder.
        HealthcareManagement.addNotification(recipient, "Appointment Reminder: " + message);
    }
}
